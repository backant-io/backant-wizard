...
export class AmpMCPClient extends MCPClient {
  ...
  async isServerInstalled(): Promise<boolean> {
    ...
    if (binary) {
      try {
        const result = spawnSync(binary, ['mcp', 'list'], { encoding: 'utf-8', stdio: 'pipe' });
        if (result.stdout?.includes(SERVER_NAME)) return true;
      } catch {}
    }
  ...
  async addServer(apiKey: string, mode: 'local' | 'remote'): Promise<MCPClientResult> {
    ...
    if (binary && mode === 'remote') {
      const args = ['mcp', 'add', SERVER_NAME, '--header', `Authorization=Bearer ${apiKey}`, REMOTE_MCP_URL];
      debug(`Running: ${binary} ${args.join(' ')}`);
      const result = spawnSync(binary, args, { stdio: 'pipe' });
      if (!result.error && result.status === 0) return { success: true };
    }
    return this.addServerViaConfigFile(apiKey, mode);
  }

  private async addServerViaConfigFile(apiKey: string, mode: 'local' | 'remote'): Promise<MCPClientResult> {
    const configPath = this.getConfigFilePath();
    ...
    try {
      ...
      (config.mcpServers as Record<string, unknown>)[SERVER_NAME] = this.getServerConfig(apiKey, mode);
  ...
  async removeServer(): Promise<MCPClientResult> {
    const binary = this.findAmpBinary();
    if (binary) {
      const result = spawnSync(binary, ['mcp', 'remove', SERVER_NAME], { stdio: 'pipe' });
      if (!result.error && result.status === 0) return { success: true };
    }
    const configPath = this.getConfigFilePath();

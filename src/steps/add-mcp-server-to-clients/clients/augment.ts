...
export class AugmentMCPClient extends MCPClient {
  ...
  async isClientSupported(): Promise<boolean> {
    ...
    if (platform === 'darwin') {
      extensionDir = path.join(os.homedir(), 'Library', 'Application Support', 'Code', 'User', 'globalStorage', 'augment.augment-vscode');
    } else if (platform === 'win32') {
      extensionDir = path.join(process.env.APPDATA || '', 'Code', 'User', 'globalStorage', 'augment.augment-vscode');
    } else {
      extensionDir = path.join(os.homedir(), '.config', 'Code', 'User', 'globalStorage', 'augment.augment-vscode');
    }
